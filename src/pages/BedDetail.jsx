...
export default function BedDetail() {
  ...
  return (
    <div className="space-y-4">
      <Link to="/beete" className="p-2 -ml-2 text-slate-500 hover:text-slate-800 inline-block"><ArrowLeft className="w-5 h-5" /></Link>
      <div>
        <h2 className="text-xl font-bold">{bed.name}</h2>
        <p className="text-sm text-slate-500">{bed.dimensions.length}×{bed.dimensions.width} cm • {sunLabels[bed.sunExposure]}</p>
      </div>
      {bed.notes && <div className="bg-white border border-slate-200 rounded-xl p-4"><p className="text-sm text-slate-700">{bed.notes}</p></div>}
      ...
      {bedTasks.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {bedTasks.map((t) => <div key={t.id} className="text-sm p-2 bg-slate-50 rounded-lg">{t.title}</div>)}
          </div>
        </div>
      )}
    </div>
