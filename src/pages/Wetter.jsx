...
export default function Wetter() {
  ...
  if (loading) return <div className="space-y-4"><div className="bg-slate-800 rounded-xl h-32 animate-pulse" /><div className="bg-slate-800 rounded-xl h-64 animate-pulse" /></div>
  if (error) return <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-red-400 text-sm">{error}</div>
          ...
          {forecast.map((day) => {
            ...
            return (
              <div key={day.date} className="flex items-center justify-between py-2 border-b border-slate-700/50 last:border-0">
                ...
                <div className="w-16 text-right text-xs text-slate-400">{day.precipitation > 0 ? `${day.precipitation} mm` : '—'}</div>
              </div>
            )
          })}
        </div>
